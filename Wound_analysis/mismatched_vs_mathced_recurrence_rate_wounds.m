function []= mismatched_vs_mathced_recurrence_rate_wounds(wound_cases,params)
...
for drug = 1:params.number_drugs  
   ...
   all_sensitive = (ismember(wound_cases.SMP_Res(:,drug), params.sensitive_group))  ;
   all_reistant = (ismember(wound_cases.SMP_Res(:,drug), params.resistant_group))  ;
      
   all_sensitive_nextres =  ismember(wound_cases.next_res(:,drug),params.sensitive_group);
   all_resistant_nextres =  ismember(wound_cases.next_res(:,drug),params.resistant_group);
   all_nextres = ismember(wound_cases.next_res(:,drug),[1 2 3]);
   
   all_sensitive_purchased(drug) =  nnz(all_sensitive & wound_cases.PCR_sameday(:,drug));
   sensitive_purchased_fails(drug) = nnz(all_sensitive & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure);
   sensitive_purchased_nextresfails(drug) = nnz(all_sensitive & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure & all_nextres);
   
   sensitive_purchased_fails_SS(drug) = nnz(all_sensitive & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure & all_sensitive_nextres);
   sensitive_purchased_fails_SR(drug) = nnz(all_sensitive & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure & all_resistant_nextres);
   
   all_resistant_purchased(drug) =  nnz(all_reistant & wound_cases.PCR_sameday(:,drug));
   resistant_purchased_fails(drug) = nnz(all_reistant & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure);
   resistant_purchased_nextresfails(drug) = nnz(all_reistant & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure & all_nextres);
   
   resistant_purchased_fails_RS(drug) = nnz(all_reistant & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure & all_sensitive_nextres);
   resistant_purchased_fails_RR(drug) = nnz(all_reistant & wound_cases.PCR_sameday(:,drug) & wound_cases.treatfailure & all_resistant_nextres);
end
...
ylabel({'percentage of wound infections' ; 'resulting in early recurrence'})
set(gca,'XTick',[1 2]);
